...
class AccountSalesViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
    ...
    filter_fields = ('platform', 'platform_base', 'account_name', 'manager',
                     'month')  # 配置过滤字段
    ...
    def update(self, request, pk=None):
        serialized = AccountSalesSerializerNodepth(self.get_object(),
                                                   data=request.data)
        if serialized.is_valid():
            serialized.save()
            return Response(serialized.data)
...
class AccountBonusViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                          mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                          mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
    ...
    def create_bonus(self, request):
        ...
        for i in account_sales:
            account_bonus = AccountBonus.objects.filter(
                month=month, account_name=i.account_name,
    ...
    def export_bonus(self, request):
        ...
        border = Border(left=Side(border_style='thin', color='e7e6e6'),
        ...
        for m in months:
            for p in platforms:
                ab = AccountBonus.objects.filter(
                    month=m, platform=p, manager__name=manager).count()
                if ab:
                    is_exist = True
                    break
        if not is_exist:
            return Response({'msg': '无数据'},
        ...
        for m in months:
            for p in platforms:
                if p == 'eBay' or p == 'Aliexpress':
                    ...
                    account_bonus = AccountBonus.objects.filter(
                        month=m, platform=p, manager__name=manager)
                    if account_bonus:
                        f_sheet = wb.create_sheet(m[0:4] + '年' + m[4:6] + '月' +
                                                  p)
                        num = 0
                        for ab in account_bonus:
                            ...
                            if num == 0:
                                ...
                                f_sheet['A1'] = m[0:4] + '年' + m[
                                    4:6] + '月' + p + '销售报表-' + ab.manager.name
                                # 第1行样式
                                ...
                                f_sheet['A1'].fill = PatternFill(
                                    patternType='solid', fgColor='65d6b4')
                                for i in area:
                                ...
                                for i in area:
                                    ...
                                    f_sheet[i + '2'].fill = PatternFill(
                                        patternType='solid', fgColor='df8244')
                                ...
                                f_sheet[
                                    'C3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.sale_amount).humanized_amount(
                                            compel=True)
                                f_sheet[
                                    'D3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.receipts).humanized_amount(
                                            compel=True)
                                f_sheet[
                                    'E3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.refund).humanized_amount(
                                            compel=True)
                                f_sheet['F3'] = '￥' + xstring.dispose(
                                    ab.FES).humanized_amount(compel=True)
                                f_sheet[
                                    'G3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.platform_fees).humanized_amount(
                                            compel=True)
                                f_sheet['H3'] = '￥' + xstring.dispose(
                                    ab.product_cost).humanized_amount(
                                        compel=True)
                                f_sheet['I3'] = '￥' + xstring.dispose(
                                    ab.shipping_cost).humanized_amount(
                                        compel=True)
                                f_sheet['J3'] = '￥' + xstring.dispose(
                                    ab.profit).humanized_amount(compel=True)
                                f_sheet['K3'] = ab.orders
                                f_sheet[
                                    'L3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.CUP).humanized_amount(compel=True)
                                f_sheet[
                                    'M3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.ad_fees).humanized_amount(
                                            compel=True)
                                f_sheet['N3'] = xstring.dispose(
                                    ab.ad_percent).humanized_amount(
                                        compel=True) + '%'
                                f_sheet['O3'] = '￥' + xstring.dispose(
                                    ab.bonus).humanized_amount(
                                        compel=True) if ab.bonus > 0 else 0
                                ...
                                for i in area:
                                    f_sheet[i +
                                            str(num + 3)].alignment = alignment
                                    f_sheet[i +
                                            str(num + 3)].font = title_style3
                                    f_sheet[i + str(num + 3)].border = border
                                if ab.bonus < 0:
                                    f_sheet['J' +
                                            str(num +
                                                3)].font = title_style3_red
                                f_sheet['A' + str(num + 3)] = ab.account_name
                                f_sheet['B' + str(num + 3)] = ab.platform_base
                                f_sheet['C' + str(
                                    num + 3
                                )] = ab.ori_currency + ' ' + xstring.dispose(
                                    ab.sale_amount).humanized_amount(
                                        compel=True)
                                f_sheet['D' + str(
                                    num + 3
                                )] = ab.ori_currency + ' ' + xstring.dispose(
                                    ab.receipts).humanized_amount(compel=True)
                                f_sheet['E' + str(
                                    num + 3
                                )] = ab.ori_currency + ' ' + xstring.dispose(
                                    ab.refund).humanized_amount(compel=True)
                                f_sheet['F' +
                                        str(num + 3)] = '￥' + xstring.dispose(
                                            ab.FES).humanized_amount(
                                                compel=True)
                                f_sheet['G' + str(
                                    num + 3
                                )] = ab.ori_currency + ' ' + xstring.dispose(
                                ...
                                f_sheet['M' + str(
                                    ...
                                    ab.ad_fees).humanized_amount(compel=True)
                                f_sheet['N' + str(num + 3)] = xstring.dispose(
                                    ab.ad_percent).humanized_amount(
                                        compel=True) + '%'
                                f_sheet[
                                    'O' +
                                    str(num + 3)] = '￥' + xstring.dispose(
                                        ab.bonus).humanized_amount(
                                            compel=True) if ab.bonus > 0 else 0
                                f_sheet['P' + str(
                                    num + 3)] = '是' if ab.bonus >= 500 else '否'
                                if ab.bonus >= 500:
                                    f_sheet['P' +
                                            str(num + 3)].font = title_style3_b
                                else:
                                    f_sheet['P' +
                                            str(num +
                                                3)].font = title_style3_red
                    ...
                    if account_bonus:
                        ...
                        for ab in account_bonus:
                            ...
                            if num == 0:
                                ...
                                f_sheet['A1'] = m[0:4] + '年' + m[
                                    4:
                                    6] + '月' + 'Coupang销售报表-' + ab.manager.name
                                # 第1行样式
                                f_sheet.row_dimensions[1].height = 50
                                f_sheet.row_dimensions[2].height = 30
                                f_sheet['A1'].alignment = alignment
                                ...
                                f_sheet[
                                    'E3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.cp_cgf_fees).humanized_amount(
                                            compel=True)
                                f_sheet[
                                    'F3'] = ab.ori_currency + ' ' + xstring.dispose(
                                        ab.receipts).humanized_amount(
                                            compel=True)
                                f_sheet['G3'] = '￥' + xstring.dispose(
                                    ab.FES).humanized_amount(compel=True)
                                f_sheet['H3'] = '￥' + xstring.dispose(
                                    ab.product_cost).humanized_amount(
                                        compel=True)
                                f_sheet['I3'] = '￥' + xstring.dispose(
                                    ab.cp_first_ship).humanized_amount(
                                        compel=True)
                                f_sheet['J3'] = '￥' + xstring.dispose(
                                    ab.profit).humanized_amount(compel=True)
                                f_sheet['K3'] = '￥' + xstring.dispose(
                                    ab.bonus).humanized_amount(compel=True)
                                f_sheet['L3'] = '是'
                            elif num > 0:
                                # >=4行样式
                                for i in area2:
...
class AccountsViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                      mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                      mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
    ...
    # 重写update，让depth序列号字段能保存数据
    def update(self, request, pk=None):
        serialized = AccountsSerializerNodepth(self.get_object(),
                                               data=request.data)
        if serialized.is_valid():
            serialized.save()
            return Response(serialized.data)
...
class MonthListViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                       mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                       mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
...
class ExchangeRateViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                          mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                          mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
...
class BasicInfoViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                       mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                       mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
...
class ManagerViewSet(mixins.ListModelMixin, mixins.CreateModelMixin,
                     mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                     mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    list:
    ...
    filter_backends = (DjangoFilterBackend, filters.SearchFilter)  # 过滤,搜索,排序
    filter_fields = ('is_active', )  # 配置过滤字段
